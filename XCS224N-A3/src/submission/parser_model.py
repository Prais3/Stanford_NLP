...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, t):
        ...
        x = embeddings.view(embeddings_size[0], embeddings_size[1] * embeddings_size[2])
        ### END CODE HERE
        return x
    ...
    def forward(self, t):
        ...
        embed_to_hidden_calc = self.embed_to_hidden(embeddings)
        hidden_units = nn.functional.relu(embed_to_hidden_calc)
        after_dropout = self.dropout(hidden_units)
        logits = self.hidden_to_logits(after_dropout)
        ### END CODE HERE
        return logits
