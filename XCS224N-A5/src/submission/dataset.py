...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rand = random.randint(4, int(self.block_size*7/8))
        document = document[:rand]

        get_prefix = random.randint(1, len(document)-2)
        prefix = document[:get_prefix]

        get_suffix = random.randint(get_prefix+1, len(document)-1)
        suffix = document[get_suffix:]

        masked_content = document[get_prefix:get_suffix]

        result = prefix + self.MASK_CHAR + suffix  + self.MASK_CHAR + masked_content
        ...
        for cx, cy in zip(x, y):
            stoi_x.append(self.stoi[cx])
            stoi_y.append(self.stoi[cy])

        x, y = torch.tensor(stoi_x, dtype=torch.long), torch.tensor(stoi_y, dtype=torch.long)
