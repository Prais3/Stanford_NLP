...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  get_y_hat = softmax(np.dot(outside_vectors, center_word_vec))

  loss = -np.log(get_y_hat[outside_word_idx])
  
  outside_vectors_transpose = np.transpose(outside_vectors)
  get_y_hat[outside_word_idx] -= 1

  grad_center_vec = np.dot(outside_vectors_transpose, get_y_hat)

  reshape_y_hat = np.reshape(get_y_hat, (-1, 1))
  get_shape = center_word_vec.shape
  reshape_center_vec = np.reshape(center_word_vec, (-1, get_shape[0]))

  grad_outside_vecs = np.dot(reshape_y_hat, reshape_center_vec)
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  ### START CODE HERE
  get_center_word_index = word2ind[current_center_word]

  for outside_word in outside_words:
      get_outside_word_index =  word2ind[outside_word]
      loss_current, gradc, grado = word2vec_loss_and_gradient(center_word_vectors[get_center_word_index], get_outside_word_index, outside_vectors, dataset)
      loss += loss_current
      grad_center_vecs[get_center_word_index] += gradc
